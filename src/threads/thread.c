...
findHolderLocks(struct thread * h){
	...
	if(!list_empty(&h->request)){
				...
				e = list_next (e))
		{
		  ...
		  //printf("TEST: %s\n",l->holder->name);
		  //ASSERT(l->holder->name != NULL);
		  ///if(l->holder != h){
			  testsearch(l->holder);
		  //}
		  //findTHolder(l->holder, thread_current());
...
findTHolder(struct thread * h, struct thread * cur){
	...
	if(!list_empty(&h->request)){
		//struct lock * l = list_entry (list_front(&h->request), struct lock, elem);
		
		//printf("A: %s", l->holder->name);
		findHolderLocks(h);
	}
...
thread_yield (void)
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread){
	//printf("SSS: %d",list_size(&ready_list));
	list_insert_ordered(&ready_list, &cur->elem, priority_compare, NULL); 
	//printf("\nIIIIIII: %s\n",cur->name);
  ...
  schedule ();
  intr_set_level (old_level);
  //printf("SIZEYIELD: %d",list_size(&ready_list));
}
